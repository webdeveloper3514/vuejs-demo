    ...
    return new Promise((resolve) => {
      ...
      for (i = 0; i < window.localStorage.length; i++) {
        ...
        if (
          (key === null || key === void 0
            ? void 0
            : key.substring(0, this.prefix.length)) == this.prefix
        ) {
          storedKeys.push(key);
        }
      }
        ...
        if (before) {
          try {
            const storedItem = JSON.parse(
              window.localStorage.getItem(storedKeys[i])
            );
...
const unfetch_module$1 = /*#__PURE__*/ Object.freeze({
  __proto__: null,
...
const browser = self.fetch || (self.fetch = require$$0.default || require$$0);

const fromByteArray_1 = fromByteArray;
...
function deriveChallenge(code) {
  if (code.length < 43 || code.length > 128) {
    return Promise.reject(
      new OIDCClientError(`Invalid code length: ${code.length}`)
    );
  }
  return new Promise((resolve, reject) => {
...
function validateJwt(jwt, options, isIdToken = false) {
  ...
  const now =
    ((currentTimeInMillis && currentTimeInMillis()) || Date.now()) / 1000;
  ...
  if (
    ...
    throw new InvalidJWTError(
      `Invalid Audience (aud) in token: ${payload.aud}`
    );
  }
  if (payload.azp && payload.azp !== client_id) {
    throw new InvalidJWTError(
      `Invalid Authorized Party (azp) in token: ${payload.azp}`
    );
  }
  ...
  //   throw new InvalidJWTError(
  //     `Issued At (iat) is in the future: ${payload.iat}`
  //   );
  // }
  if (payload.nbf && lowerNow < payload.nbf) {
    throw new InvalidJWTError(
      `Not Before time (nbf) is in the future: ${payload.nbf}`
    );
  }
  if (!payload.exp) {
  ...
  if (payload.exp < upperNow) {
    throw new InvalidJWTError(
      `Expiration Time (exp) is in the past: ${payload.exp}`
    );
  }
  return payload;
}
      ...
      if (sessionState && internalSessionState !== sessionState) {
        ...
        const send = () => {
          frame.contentWindow.postMessage(
            `${client_id} ${internalSessionState}`,
            frameOrigin
          );
        };
        send();
...
const isResponseType = (type, response_type) =>
  response_type &&
  response_type.split(/\s+/g).filter((rt) => rt === type).length > 0;
const isScopeIncluded = (scope, scopes) =>
...
function runPopup(url, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function messageListener(e) {
      ...
      data.error
        ? reject(new OIDCClientError(data.error, data.error_description))
        : resolve(e.data);
    }
  });
}
...
class TabUtils {
  ...
  BroadcastMessageToAllTabs(messageId, eventData) {
    ...
    localStorage.setItem(
      `${this.keyPrefix}event${messageId}`,
      JSON.stringify(data)
    );
    //cleanup
    setTimeout(() => {
...
class OIDCClient extends EventEmitter {
  constructor(options) {
    ...
    this.options = Object.assign(
      ...
      options,
      {
        ...
        issuer: options.issuer.endsWith("/")
          ? options.issuer.slice(0, -1)
          : options.issuer,
      }
    );
    ...
    this.synchronizer.OnBroadcastMessage(
      Events.USER_LOGIN,
      ...
      this.__initializePromise = new Promise(async (resolve, reject) => {
        try {
          ...
          if (
            ...
            Object.keys(this.options.endpoints).length === 0
          ) {
   ...
   */ async loginWithPopup(options = {}, popupOptions = {}) {
    ...
    const { authParams, localState } =
      !state || typeof state === "string"
        ...
        : state;
    const tokenResult = await this.handleAuthResponse(
      response,
      authParams,
      localState
    );
   ...
   */ async loginCallback(
    url = window === null || window === void 0
      ? void 0
      : (ref = window.location) === null || ref === void 0
      ? void 0
      : ref.href
  ) {
    if (!url) {
      return Promise.reject(
        new OIDCClientError("Url must be passed to handle login redirect")
      );
    }
    let parsedUrl;
    try {
      ...
      return Promise.reject(
        new OIDCClientError(`Invalid callback url passed: "${url}"`)
      );
    }
    ...
    switch (request_type) {
      case "s":
        if (
          window === null || window === void 0 ? void 0 : window.frameElement
        ) {
          if (url) {
   ...
   */ async logout(options = {}) {
    if (!options.localOnly) {
      ...
      const id_token_hint =
        ...
        (storedAuth === null || storedAuth === void 0
          ? void 0
          : storedAuth.id_token_raw);
      const endpoint_for_logout = await this.createLogoutRequest({
        ...options,
        id_token_hint,
      });
      await this.authStore.clear();
      return endpoint_for_logout;
      // window.location.assign(endpoint_for_logout);
    }
  }
   ...
   */ async revokeToken(token, type = "access_token", options = {}) {
    if (!this.options.endpoints.revocation_endpoint) {
      return Promise.reject(
        new OIDCClientError('"revocation_endpoint" doesn\'t exist')
      );
    }
    const finalOptions = {
   ...
   */ async silentLogin(options = {}, localState = {}) {
    ...
    if (
      this.options.useRefreshToken &&
      (storedAuth === null || storedAuth === void 0
        ? void 0
        : storedAuth.refresh_token)
    ) {
      // TODO: deep merge
      finalState.authParams = Object.assign(
        {},
        (storedAuth === null || storedAuth === void 0
          ? void 0
      ...
      tokenResult = await this.handleAuthResponse(
        response,
        finalOptions,
        localState
      );
   ...
   */ async getAccessToken() {
    let ref;
    return (ref = await this.authStore.get("auth")) === null || ref === void 0
      ? void 0
      : ref.access_token;
  }
   ...
   */ async getScopes() {
    ...
    return (ref = await this.authStore.get("auth")) === null || ref === void 0
      ? void 0
      : (ref1 = ref.scope) === null || ref1 === void 0
      ? void 0
      : ref1.split(" ");
  }
   ...
   */ async createAuthRequest(options = {}, localState = {}) {
    let ref;
    if (
      !((ref = this.options.endpoints) === null || ref === void 0
        ? void 0
        : ref.authorization_endpoint)
    ) {
      await this.initialize(false);
    }
    ...
    if (isResponseType("code", authParams.response_type)) {
      authParams.code_challenge = await deriveChallenge(
        localState.code_verifier
      );
      authParams.code_challenge_method =
        finalOptions.code_challenge_method || "S256";
    }
    const now =
      (this.options.currentTimeInMillis &&
        this.options.currentTimeInMillis()) ||
      Date.now();
   ...
   */ async createLogoutRequest(options = {}) {
    ...
    return `${
      this.options.endpoints.end_session_endpoint
    }${buildEncodedQueryString(logoutParams)}`;
  }
   ...
   */ async handleTokenResult(tokenResult, authParams, finalOptions) {
    ...
    if (tokenResult.id_token) {
      parsedIDToken = await validateIdToken(
      ...
      if (
        ...
        return Promise.reject(
          new InvalidIdTokenError("Id Token validation failed")
        );
      }
    ...
    if (tokenResult.access_token) {
      let ref;
      if (
        finalOptions.requestUserInfo &&
        ((ref = this.options.endpoints) === null || ref === void 0
          ? void 0
          : ref.userinfo_endpoint)
      ) {
   ...
   */ monitorSession({ sub, session_state }) {
    ...
    if (
      !(endpoints === null || endpoints === void 0
        ? void 0
        : endpoints.check_session_iframe)
    ) {
      console.warn(
  ...
  async onUserLogin(authObj) {
    ...
    this.user = user;
    this.scopes =
      scope === null || scope === void 0 ? void 0 : scope.split(" ");
    ...
    if (
      ...
      if (expires_in !== undefined && this.options.autoSilentRenew) {
        const expiration =
          Number(expires_in) -
          this.options.secondsToRefreshAccessTokenBeforeExp;
        if (expiration >= 0) {
